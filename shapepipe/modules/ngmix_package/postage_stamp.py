...
def preprocess_postage_stamps(self, vignet, tile_cat, obj_id):
    ...
    for exp_ccd in epoch_list:
        ...
        # filptile megacam images to mactch ccd if necessary
        ...
        gal_masked, weight, noise_img = _prepare_ngmix_weights(
            gal_sub_bkg,
            weight_raw,
            flag
        )
        ...
        stamp.gals.append(gal_scaled)
        stamp.weights.append(weight_scaled)
        stamp.flags.append(flag)
        stamp.jacobs.append(jacob)
        stamp.noise_ims.append(noise_img)
        stamp.psfs.append(
...
def _MegaCamFlip(self, vign, ccd_nb):
    """Flip for MegaCam.
